...
<div class="space-y-6">
    <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
            <h1 class="page-title">Notifikasi Admin</h1>
            <p class="page-subtitle">Semua tindakan terbaru dari siswa akan muncul di sini agar proses verifikasi lebih cepat.</p>
        </div>
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <form method="POST" action="{{ route('admin.notifications.destroy-all') }}" onsubmit="return confirm('Yakin ingin menghapus semua notifikasi admin?')">
                @csrf
                @method('DELETE')
    ...
    <div class="table-wrap">
        <form method="POST" action="{{ route('admin.notifications.destroy-selected') }}" onsubmit="return confirm('Hapus notifikasi admin yang dipilih?')">
            @csrf
            @method('DELETE')
                    ...
                    <div class="flex gap-3 px-4 py-4 transition hover:bg-slate-50/80 sm:px-6 {{ $isUnread ? 'bg-sky-50/40' : '' }}">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-2">
                                <p class="font-semibold text-slate-900">{{ $notification->data['title'] ?? 'Notifikasi Admin' }}</p>
                                @if($isUnread)
